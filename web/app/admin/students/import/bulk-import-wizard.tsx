...
import { commitBulkImport, previewBulkImport } from '@/lib/actions/bulk-import';
import type { AcademicSessionDto } from '@/lib/types/academic';
import type { BulkImportCommitResponse, BulkImportPreviewResponse } from '@/lib/types/students';
import { cn } from '@/lib/utils';
import { ImportPreviewTable } from './import-preview-table';

const STEPS = ['Upload', 'Review', 'Confirm'];

export function BulkImportWizard({ sessions }: { sessions: AcademicSessionDto[] }) {
  ...
  async function handleUploadContinue() {
    if (!termId) {
      toast.error('Choose which term to enroll these students into.');
      return;
    }
    if (!file) {
    ...
    try {
      ...
      const result = await previewBulkImport(formData, termId);
      setPreview(result);
      setStep(2);
    } catch (error) {
  ...
  async function handleConfirm() {
    ...
    setIsLoading(true);
    try {
      const result = await commitBulkImport(termId, preview.valid);
        ...
        <CardContent className="space-y-4 text-center">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              {commitResult.succeededCount} student{commitResult.succeededCount === 1 ? '' : 's'}{' '}
          ...
          <Button render={<Link href="/admin/students" />}>Back to Student Directory</Button>
        </CardContent>
      </Card>
    );
  }
        ...
        <Card>
          <CardContent className="space-y-6">
            <div className="space-y-2">
              <Label>Enroll into Term</Label>
              <Select
                value={termId}
                onValueChange={(v) => v && setTermId(v)}
